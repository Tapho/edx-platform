...
class NumberedCanvas(Canvas):  # pylint: disable=abstract-method
    ...
    def draw_page_number(self, page_count):
        ...
        self.drawRightString(200 * mm, 12 * mm,
                             _("Page %(page_number)d of %(page_count)d") % {"page_number": self._pageNumber,
...
class PDFInvoice(object):
    ...
    def __init__(self, context):
        """
        The context is expected to be a dict having the following items:
        ...
        self.cobrand_logo_path = microsite.get_value("PDF_RECEIPT_COBRAND_LOGO_PATH", settings.PDF_RECEIPT_COBRAND_LOGO_PATH)
        ...
        self.billing_address_text = microsite.get_value("PDF_RECEIPT_BILLING_ADDRESS", settings.PDF_RECEIPT_BILLING_ADDRESS)
        self.terms_conditions_text = microsite.get_value("PDF_RECEIPT_TERMS_AND_CONDITIONS", settings.PDF_RECEIPT_TERMS_AND_CONDITIONS)

        # From Context
        self.items_data = context['items_data']
        self.invoice_id = context['id']
        self.date = ModuleI18nService().strftime(context['date'], 'SHORT_DATE')
        self.is_invoice = context['is_invoice']
        self.total_cost = '{currency}{amount:.2f}'.format(currency=self.currency, amount=context['total_cost'])
        self.payment_received = '{currency}{amount:.2f}'.format(currency=self.currency, amount=context['payment_received'])
        self.balance = '{currency}{amount:.2f}'.format(currency=self.currency, amount=context['balance'])
    ...
    def draw_logos(self):
        ...
        if logo_img:
            ...
            self.pdf.drawImage(logo_img.filename,
                               self.page_width - (horizontal_padding_from_border + img_width), img_y_pos,
    ...
    def draw_title(self, y_pos):
        ...
        font_size = 10
        self.pdf.setFontSize(font_size)
        horizontal_padding_from_border = 11 * mm
        y_pos = y_pos - font_size / 2 - vertical_padding
        # Draw Order/Invoice No.
        self.pdf.drawString(self.margin + horizontal_padding_from_border, y_pos,
                            _(u'{id_label} # {invoice_id}'.format(id_label=id_label, invoice_id=self.invoice_id)))
        # Draw Date
        self.pdf.drawRightString(self.page_width - (self.margin + horizontal_padding_from_border), y_pos,
                                 _(u'Date: {date}'.format(date=self.date)))
    ...
    def draw_course_info(self, y_pos):
        ...
        course_items_table = Table(course_items_data,
                                   [padding_width, desc_col_width, qty_col_width, list_price_col_width,
                                    discount_col_width, amount_col_width, padding_width], splitByRow=1, repeatRows=1)
    ...
    def draw_footer(self, y_pos):
        ...
        if self.is_invoice:
            ...
            # footer_style.append(('RIGHTPADDING', (1, 6), (1, 6), 5 * mm))
            footer_style.append(('BOX', (1, 6), (1, 6), 0.50, colors.black))

        # Styling for the entire footer table.
        footer_style.append(('ALIGN', (0, 0), (-1, -1), 'LEFT'))
        footer_style.append(('VALIGN', (0, 0), (-1, -1), 'MIDDLE'))
        footer_style.append(('TEXTCOLOR', (0, 0), (-1, -1), colors.black))
